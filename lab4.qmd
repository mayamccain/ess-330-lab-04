...
salamander_table <- table(salamander_clean$section)
```
...
```
There is a significant difference in species biomass between clear cut and old growth forest types for the Coastal Giant salamander (p-value < 2.2e-16). 
...
trout_body_clean <- and_vertebrates  |>  
  filter(species == "Cutthroat trout") |> 
  select(length_1_mm, weight_g) |>
  drop_na() |>
cor() 

print(trout_body_clean)

```

The correlation is `r trout_body_clean[2,1]`. There is a strong positive relationship between body length and body mass in Cutthroat trout. A correlation of 0.912 is very close to 1, showing this correlation. 
...
```{r}
pie_sites |> 
  ggboxplot(x = 'latitude', y = 'size') +
  geom_jitter(size =.25) + 
  theme(legend.postition = "none")
```
The ANOVA test shows that at least one of our site means is significantly different from the others (p-value < 2.2e-16) while the post-hoc Tukey test determines which groups are different from each other in carapace width based on latitude. Sites GTM and DB are statistically different from each other (p adj < 0.05, p adj = 3.0e-07). All sites included in these tests are statistically different from each other shown by the small p adj values (PIE-DB p adj = 2.9e-06, PIE-GTM p adj = 0.0e+00). We can also detect these differences using the visual boxplot representation of the model. 
...
```{r}
pie_temp_lm <- lm(size ~ water_temp_sd, data = pie_crab)
...
pie_crab |> 
  ggscatter(x = 'water_temp_sd', y = 'size', 
            ...
            add = "reg.line")
```
The linear regression model shows that there is not a significant impact of water temperature on crab carapace width (Multiple R-squared:  0.001024). This model is not statistically significant (p-value = 0.5275).
...
```{r}
pie_effects_mlm <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(pie_effects_mlm)
```
```{r}
pie_crab |> 
  ...
  cor()
```
The multiple linear regression model is statistically significant (p-value < 2.2e-16), showing the combination of variables accounts for 35% of variation (Multiple R-squared:  0.3516). Latitude is the most significant factor influencing crab carapace width (p-val < 2e-16). The model shows that there is not a significant impact of the combination of latitude, air temperature, and water temperature on carapace width. 
